...
function fmtAU(dateIso: string) {
  ...
  return d.toLocaleDateString('en-AU', { weekday: 'short', day: '2-digit', month: 'short' }); // e.g., Mon 13 Oct
}
...
export default function PlanPage() {
  ...
  // Household
  useEffect(() => {
    supabase.rpc('get_my_household').then(({ data }) => {
      if (data) setHouseholdId(data);
    });
  }, []);

  // Ensure plan exists for week
  useEffect(() => {
    const ensure = async () => {
  ...
  async function setDinner(date: string, recipeId: string) {
    ...
    try {
      ...
      if (items && items.length > 0) {
        ...
        await supabase.from('meal_plan_items').update({ recipe_id: recipeId }).eq('id', keepId);
        if (toDelete.length) {
          await supabase.from('meal_plan_items').delete().in('id', toDelete);
        }
      } else {
        await supabase
          .from('meal_plan_items')
          .insert({ meal_plan_id: mealPlanId, plan_date: date, slot: 'dinner', recipe_id: recipeId });
      }
      await reloadWeek();
      setEditing(null);
    } finally {
  ...
  async function setLunch(date: string, lunchId: string) {
    ...
    try {
      ...
      let itemId: string | undefined = items?.[0]?.id;
      ...
      if (!itemId) {
        ...
        const toDelete = (items ?? []).slice(1).map((i) => i.id);
        if (toDelete.length) {
          await supabase.from('meal_plan_lunch_components').delete().in('meal_plan_item_id', toDelete);
          await supabase.from('meal_plan_items').delete().in('id', toDelete);
        }
      }

      await supabase.from('meal_plan_lunch_components').delete().eq('meal_plan_item_id', itemId!);
      const add = await supabase
        .from('meal_plan_lunch_components')
        .insert({ meal_plan_item_id: itemId!, lunch_component_id: lunchId, servings: 1 });
      if (add.error) {
  ...
  async function clearSlot(date: string, slot: 'dinner' | 'lunch') {
    ...
    try {
      ...
      const { error: delItems } = await supabase.from('meal_plan_items').delete().in('id', ids);
      if (delItems) {
        alert(delItems.message);
        return;
      }
  ...
  const filteredRecipes = recipes.filter((r) =>
    qDinner ? r.title.toLowerCase().includes(qDinner.toLowerCase()) : true
  );
  const filteredLunches = lunches.filter((l) =>
    qLunch ? l.name.toLowerCase().includes(qLunch.toLowerCase()) : true
  );

  const weekDaysOverview = weekDays.map((d) => {
  ...
  const generateList = async () => {
    if (!householdId) {
      alert('No household linked.');
      return;
    }
    const { data, error } = await supabase.rpc('create_shopping_list_for_week', {
  ...
  return (
    <div className="card">
      ...
      <div className="flex flex-wrap gap-3 items-center mb-4">
        ...
        />
        <button className="btn" onClick={resetWeek} disabled={busy}>
          Reset week
        </button>
        <button className="btn" onClick={() => window.print()}>
          Print week
        </button>
        <button className="btn-primary btn" onClick={generateList} disabled={busy}>
      ...
      <div className="border rounded-xl p-3 mb-4">
        ...
        <ul className="space-y-1">
          {weekDaysOverview.map((o) => (
            <li key={o.date} className="text-sm">
              {o.label}
            </li>
          ))}
        </ul>
            ...
            <div key={d} className="border rounded-xl p-3">
              ...
              <div className="mb-3">
                <div className="text-sm mb-1">
                  <span className="badge">Dinner</span> {dinner || <em>—</em>}
                </div>
                ...
                {!isEdD ? (
                  dinner ? (
                    <div className="flex gap-2">
                      <button
                        className="btn flex-1 min-w-[120px] whitespace-nowrap"
                        onClick={() => setEditing({ slot: 'dinner', date: d })}
                      >
                        Change
                      </button>
                      <button
                        className="btn flex-1 min-w-[120px] whitespace-nowrap"
                        onClick={() => clearSlot(d, 'dinner')}
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      value={qDinner}
                      onChange={(e) => setQDinner(e.target.value)}
                    />
                    <ul className="max-h-48 overflow-auto border rounded-xl">
                      ...
                      {filteredRecipes.length === 0 && (
                        <li className="px-3 py-2 text-sm text-gray-500">No recipes</li>
                      )}
                    </ul>
                    <div className="mt-2">
                      <button className="btn w-full" onClick={() => setEditing(null)} disabled={busy}>
                        Cancel
                      </button>
                    </div>
                  </div>
              ...
              <div>
                <div className="text-sm mb-1">
                  <span className="badge">Lunch</span> {lunch || <em>—</em>}
                </div>
                ...
                {!isEdL ? (
                  lunch ? (
                    <div className="flex gap-2">
                      <button
                        className="btn flex-1 min-w-[120px] whitespace-nowrap"
                        onClick={() => setEditing({ slot: 'lunch', date: d })}
                      >
                        Change
                      </button>
                      <button
                        className="btn flex-1 min-w-[120px] whitespace-nowrap"
                        onClick={() => clearSlot(d, 'lunch')}
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  <div className="mt-2">
                    <input
                      className="w-full border rounded-xl px-3 py-2 mb-2"
                      placeholder="Search lunch mains…"
                      value={qLunch}
                      onChange={(e) => setQLunch(e.target.value)}
                    />
                    <ul className="max-h-48 overflow-auto border rounded-xl">
                      ...
                      {filteredLunches.length === 0 && (
                        <li className="px-3 py-2 text-sm text-gray-500">No lunch mains</li>
                      )}
                    </ul>
                    <div className="mt-2">
                      <button className="btn w-full" onClick={() => setEditing(null)} disabled={busy}>
